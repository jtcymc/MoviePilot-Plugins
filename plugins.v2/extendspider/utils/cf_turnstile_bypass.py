...
class TurnstileSolver:
    ...
    def __init__(self, config: Optional[TurnstileConfig] = None):
        ...
        try:
            ...
            for click_attempt in range(self.config.click_max_attempts):
                try:
                    ...
                    logger.debug(f'验证按钮点击成功 (尝试 {click_attempt + 1})')
                    break
    ...
    async def solve(self, url: str, user_agent: str) -> Dict[str, str]:
        ...
        if cache_key in self._cache:
            ...
            if (datetime.now() - cache_data['timestamp']).total_seconds() < self.config.cache_timeout:
                logger.info(f"使用缓存的headers: {cache_key}")
    ...
    async def _solve_internal(self, url: str, user_agent: str) -> Dict[str, str]:
        ...
        try:
            ...
            for attempt in range(self.config.max_attempts):
                logger.debug(f'验证尝试 {attempt + 1}/{self.config.max_attempts}')
                if await self._handle_verification():
                    headers = self._extract_headers(url, user_agent)
                    self._status = "success"

                    duration = (datetime.now() - start_time).total_seconds()
                    logger.info(f'Turnstile验证完成，总用时: {duration:.2f}秒')
            ...
            if self._status != "success":
                duration = (datetime.now() - start_time).total_seconds()
                logger.warning(f'Turnstile验证失败，总用时: {duration:.2f}秒')
            await self._cleanup()
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "duration": (
                    datetime.now() - self._verification_start_time).total_seconds() if self._verification_start_time else None,
